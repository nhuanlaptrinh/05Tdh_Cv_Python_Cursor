...
Write-Host "  Device Emulator - Python Automation App" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
if ($portInUse) {
    Write-Host "⚠️  Port 8007 đang được sử dụng" -ForegroundColor Yellow
    Write-Host "Đang dừng process cũ..." -ForegroundColor Yellow
    
    try {
        $process = Get-NetTCPConnection -LocalPort 8007 -ErrorAction SilentlyContinue | 
